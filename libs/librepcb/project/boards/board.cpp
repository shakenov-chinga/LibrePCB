...
Board::Board(Project& project, const QString& filepath, bool create,
  ...
  try {
    ...
    if (create) {
      ...
      FilePath    fp = mProject.getPath().getPathTo(mRelativePath % "board.lp");
      SExpression root = SExpression::parse(
          mProject.getFileSystem().readText(mRelativePath % "board.lp"), fp);
      ...
      // load user settings
      try {
        QString     path = mRelativePath % "settings.user.lp";
        SExpression root =
            SExpression::parse(mProject.getFileSystem().readText(path),
                               mProject.getPath().getPathTo(path));
