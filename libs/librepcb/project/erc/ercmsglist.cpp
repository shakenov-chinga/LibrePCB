...
void ErcMsgList::restoreIgnoreState() {
  QString  path = "circuit/erc.lp";
  FilePath fp   = mProject.getPath().getPathTo(path);
  if (mProject.getFileSystem().fileExists(path)) {
    SExpression root =
        SExpression::parse(mProject.getFileSystem().readText(path), fp);
